...
```
Within `pm_request_autosuspend` we call `pm_runtime_mark_last_busy`, which will set
...
- One thing that caught my attention was this signature:
  ```c
  static int rpm_suspend(struct device *dev, int rpmflags) __releases(&dev->power.lock) __acquires(&dev->power.lock);
  ```
  Specifically, the attributes `__releases` and `__acquires` resolves to Clang's compile-time [thread
  safety analysis](https://clang.llvm.org/docs/ThreadSafetyAnalysis.html) attributes. I didn't know of
  this and I'll incorporate that into my codebases.

### Summarizing

In order to summarize the discussion above and consisely explain why the changes do not pose any
