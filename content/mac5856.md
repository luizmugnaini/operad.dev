...
> ```
>
> which is a similar pattern that I use for auto-restorable memory arena checkpoints in my
> codebases.

For reference, the commit SHA I'm currently in is `d2a4ec19d2a2e54c23b5180e939994d3da4a6b91` in
branch `togreg`.
...
```
We can see some sleep calls, which should put us in alert, so lets analyse all function calls done
within:
...
In order to summarize the discussion above and concisely explain why the changes do not pose any
...
The maintainers requested changes to be made with regards to the scoping of the guards, pointing
