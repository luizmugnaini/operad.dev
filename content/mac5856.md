...
date = 2026-03-19
description = "The process behind contributing to the Linux Kernel"

[taxonomies]
tags = ["programming", "linux", "kernel"]
+++

This post is a collection of reports on the [tutorials](https://flusp.ime.usp.br/kernel/) provided
for the [MAC5856 class](https://edisciplinas.usp.br/course/view.php?id=139055) for Open Source
Development.

More specifically, the tutorial pages provide a guide on how to set up a development environment for
building and developing the Linux kernel.

After years of being an Arch Linux user, I've opted for going with a Fedora installation for a
change. This required some small changes with respect to the tutorials --- all of them are documented
below.
...
- I had a very annoying problem when creating the new VM: the system would never completely
...
I only needed to follow this tutorial because of my new Linux install, however I've already had
experience setting up git's email support before, so it went quite smoothly using the Thunderbird
client setup.

## Tutorial 6 --- Sending Patches With Git and a USP Email

In this tutorial I first tried following option A (using git-credential-email), however when I tried
to authenticate, I got an error saying that "DSL 2026 OAuth Proxy has not completed the Google
verification process". Since I don't have access to the "DSL 2026 OAuth Proxy" project, I decided to
go with option B --- which involves using email-oauth2-proxy.
...
`iio_dummy_read_raw` I had to substitute `ret = IIO_VAL_INT_PLUS_MICRO` with `return
...
```c
#define CLASS(_name, var) class_##_name##_t var __cleanup(class_##_name##_destructor) = class_##_name##_constructor

#define guard(_name) CLASS(_name, __UNIQUE_ID(guard))
```
This is a very interesting pattern, the `mutex` is just part of the identifier for the
`class_mutex_t` and the corresponding procedures `class_mutex_constructor` and `class_mutex_destructor`.
The `guard` package will generate a unique name for the auxiliary variable in order to avoid name
...
> **Luiz from the future here:** In fact, there exists a `scoped_guard` macro that creates a local scope
> by using a for-loop:
>
> ```c
...
> ```
>
> In all honesty, the macro above is slightly unreadable --- which is a strangely common thing in the
> Linux kernel. However this is a very useful pattern that I use for auto-restorable memory arena
> checkpoints in my codebase, and I'm glad to see it's also used in the kernel :)

For reference, the commit I'm currently on is `d2a4ec19d2a2e54c23b5180e939994d3da4a6b91` on branch
`togreg`.

## Code Analysis

Fortunately, in the case of `mlx90614.c`, the only instances where the change to `guard` could yield
deadlocks or performance problems are those that immediately call `mlx90614_power_put`, and the
...
```
We can see some sleep calls, which should put us in alert, so let's analyse all function calls done
within:
- `rpm_drop_usage_count` manipulates `dev->power.usage_count` atomically and is very
  cheap overall.
- `trace_rpm_usage` is compile-time generated, and produces a no-op when tracing is disabled:
  ...
  ```
  Out of curiosity, this `DEFINE_EVENT` macro eventually resolves into calling `__DECLARE_TRACE`, which
  ...
  ```
  Upon further inspection, the produced functions are relatively cheap and won't cause us much
  trouble.
- `might_sleep_if` will never result in a sleep since the `__pm_runtime_suspend` is called with the
  `RPM_ASYNC` enabled in `rpmflags`.
- `rpm_suspend` is a pretty big function, however, due to our static `rpmflags` being `RPM_ASYNC | RPM_AUTO`,
  we can deduce that the procedure will simply schedule a suspend event to a separate worker thread `pm_wq`.

### Summarising

In order to summarise the discussion above and concisely explain why the changes do not pose any
problems to the codebase, I made this git commit:
```txt
...
The maintainers requested changes to be made with regard to the scoping of the guards, pointing out
that `scoped_guard` could be used and avoid any of my previous cost analyses.
...
Unfortunately, the patch was not accepted and the change was ruled "not worth it" for the merge,
